 ...
 */
bool lcd_i2c_init(I2C_HandleTypeDef *pI2cHandle,uint8_t colone,uint8_t ligne);
/**
 * @brief Set cursor position
 * @param[in] row - 0 - 4 (Row)
 ...
 * @param[in] *pI2cHandle - pointer to HAL I2C handle
 * @param[in] Colone - Number Colone of LCD
 * @param[in] Ligne - Number Ligne of LCD
 */
bool lcd_i2c_init(I2C_HandleTypeDef *pI2cHandle,uint8_t colone,uint8_t ligne)
{
  HAL_Delay(50);
...
/**
 * @brief Set cursor position
 * @param[in] row - 0 - 4 for line1 or line2 or line3 or line4 ...
 * @param[in] col - 0 - 19 (20 columns LCD)
 */
void lcd_i2c_setCursor(uint8_t row, uint8_t col)
{
  ...
  {
	  if (col != 0 && col < Colone){
		  lcd_i2c_sendCommand(LCD_SETDDRAMADDR|(col + row_offsets[row]));
	  }else{
		  lcd_i2c_sendCommand(LCD_SETDDRAMADDR|row_offsets[row]);
	  }
  }
...
{
  char stringArray[Colone];
  va_list args;
  va_start(args, str);
  vsprintf(stringArray, str, args);
  va_end(args);
  for(uint8_t i=0;  i<strlen(stringArray) && i < Colone; i++)
  {
